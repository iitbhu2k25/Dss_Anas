...
const MapLibreMap: React.FC<MapLibreMapProps> = ({
  ...
  const loadGeoTIFF = useCallback(async (
    ...
    try {
      ...
      // Read raster data
      const data: Array<TypedArray> = await image.readRasters({ pool });
  ...
  useEffect(() => {
    ...
    const currentRasterIds = new Set(selectedRasters.map(r => r.raster.id));
    ...
    selectedRasters.forEach(({ raster, visible, opacity }) => {
      ...
      if (map.getSource(`raster-source-${raster.id}`)) {
        // If source exists, update layer properties
        map.setPaintProperty(`raster-layer-${raster.id}`, 'raster-opacity', opacity);
        map.setLayoutProperty(
          `raster-layer-${raster.id}`, 
          'visibility', 
          visible ? 'visible' : 'none'
        );
      } else if (!rasterInfo || (rasterInfo && !rasterInfo.loaded && !rasterInfo.error)) {
        // Load new GeoTIFF if not already loaded or currently loading
        loadGeoTIFF(raster.source, raster.id, visible, opacity);
      }
    });
